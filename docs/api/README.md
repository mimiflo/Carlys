...
| Exercices | **Livré** — `GET /api/v1/exercises` (recherche `search`, filtres `muscleGroup`/`equipment`/`difficulty`/`type`, pagination `cursor`+`limit`, `meta.nextCursor`/`hasMore`/`total`), `GET /api/v1/exercises/:idOrSlug`, `GET /api/v1/muscle-groups` (groupes NON VIDES seulement), `GET /api/v1/equipment` — catalogue seedé (146 exercices, dont 127 illustrés : pectoraux, biceps, dos, triceps et épaules), cache Redis tolérant aux pannes, exercices non publiés jamais servis | Étape 3 ✅ |
