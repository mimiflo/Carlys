...
| Exercices | **Livré** — `GET /api/v1/exercises` (recherche `search`, filtres `muscleGroup`/`equipment`/`difficulty`/`type`, pagination `cursor`+`limit`, `meta.nextCursor`/`hasMore`), `GET /api/v1/exercises/:idOrSlug`, `GET /api/v1/muscle-groups`, `GET /api/v1/equipment` — catalogue seedé (89 exercices, dont 66 illustrés : pectoraux, biceps et dos), cache Redis tolérant aux pannes, exercices non publiés jamais servis | Étape 3 ✅ |
