...
| `PAYLOAD_TOO_LARGE` | 413 | Corps trop lourd : JSON > 1 Mo (`MAX_JSON_BODY_SIZE`) ; fichier déposé au-delà de son plafond, coupé pendant la réception (photo de repas : 5 Mio, `MEAL_PHOTO_MAX_BYTES` ; média du back-office : `MEDIA_MAX_UPLOAD_BYTES`, 20 Mio par défaut). |
| `UNSUPPORTED_MEDIA_TYPE` | 415 | Type de fichier refusé : type déclaré autre que celui attendu, ou octets qui ne le prouvent pas (photo de repas : un JPEG, signature et structure vérifiées). |
...
| Progression | **Livré** — `GET /api/v1/progress/overview?period=week\|month\|year` (totaux + volume par intervalle, **découpé dans le fuseau de la personne** : `UserProfile.timezone`, validé IANA à l'écriture et replié sur UTC s'il est illisible — un `date_trunc` en UTC coupait les soirées d'entraînement en deux barres, dont une datée de la veille), `GET /api/v1/progress/records` (records personnels), `GET /api/v1/progress/timeline` (la frise : séances, mesures, leçons groupées par jour et franchissements, fusionnés, curseur `(date, id)`), `POST /api/v1/progress/milestones` (reprise du journal de récompenses d'un appareil ; la plus ANCIENNE date gagne), `GET /api/v1/progress/lifetime` (séances terminées et semaines actives sur la VIE ENTIÈRE — des faits, jamais une règle : le moteur de récompenses du mobile reste seul à décider ce qu'est une meilleure série), `GET /api/v1/progress/exercises/:exerciseId` (progression par séance : charge maximale, volume, et depuis le 22 septembre 2026 `distanceMeters`/`durationSeconds` SOMMÉS, qui rendent la courbe cardio possible), `GET/POST /api/v1/body-metrics` (création idempotente, id appareil), `PATCH /api/v1/body-metrics/:id` (correction de la valeur, de la date, ou des deux — le type ne se corrige pas), `DELETE /api/v1/body-metrics/:id` (suppression logique rejouable) | Étape 5 ✅ |
| Nutrition | **Livré** — `GET /api/v1/nutrition/metabolism` : rapport métabolique calculé **côté serveur** (BMR Mifflin-St Jeor, TDEE par niveau d'activité, objectif calorique selon le but, macros protéines/lipides/glucides, IMC + catégorie OMS, hydratation) ; le poids provient de la **dernière mesure corporelle** (la plus récente par `measuredAt`, suppressions exclues) — corriger une mesure avec `PATCH /body-metrics/:id` déplace donc ce rapport, et corriger la seule **date** suffit à changer quelle mesure fait foi ; couplage tenu par un test e2e dans `apps/api/test/progress.e2e-spec.ts`. Le profil (sexe, naissance, taille, activité, objectif) se complète via `PATCH /users/me` ; profil incomplet → `metabolism: null` + liste `missing` | Post-Étape 7 ✅ |
| Abonnements | **Livré** — `GET /api/v1/subscriptions/me` (plan effectif + abonnement projeté), `GET /subscriptions/offers` (catalogue, prix côté serveur), `POST /subscriptions/checkout` (page de paiement Stripe, identifiant appareil comme clé d'idempotence, client Stripe réutilisé s'il est connu, n'accorde aucun droit), `POST /subscriptions/portal` (portail de gestion Stripe → `{ url }`, 409 sans client Stripe connu), `GET /api/v1/entitlements` (droits évalués **côté serveur**, expiration réévaluée à chaque lecture) ; le catalogue applique `premium_exercises` (fiche premium → 403 sans droit) — voir `docs/product/subscription-purchase.md` | Étape 6 ✅ |
...
| Administration | **Livré** — `POST /api/v1/admin/auth/login` (comptes séparés, jeton à audience dédiée 12 h, 429 après `AUTH_MAX_LOGIN_ATTEMPTS` échecs : même verrouillage que le mobile, compteur distinct), `GET /admin/auth/me`, `GET /admin/overview`, `GET /admin/users` (recherche + curseur), `GET /admin/users/:id`, `PATCH /admin/users/:id/status` (suspension = sessions révoquées), `PUT /admin/users/:id/entitlements` (attribution manuelle auditée), `GET /admin/audit-logs`, `GET /admin/exercises` (catalogue **publiés ET non publiés**, recherche, curseur), `PATCH /admin/exercises/:id/publication` — RBAC par permission (`user:read`, `user:update`, `entitlement:grant`, `exercise:read`, `exercise:publish`, `exercise:write`, `media:read`, `media:write`, `audit:read`, `community:moderate`) ; catalogue : `DELETE /admin/exercises/:id` (suppression DOUCE) + `POST /admin/exercises/:id/restore`, `PATCH /admin/exercises/:id/categories`, CRUD `/admin/muscle-groups`, `GET /admin/equipment` | Étape 7 ✅ |
| Programmes | **Livré** — `GET /api/v1/programs` (curseur), `GET /programs/:id`, `PUT /programs/:id` (**unique écriture**, create-or-replace, 201/200, `startsOn` = jour civil `AAAA-MM-JJ` facultatif — absent vaut « pas de calendrier »), `GET /programs/:id/calendar?week=` (**semaine DATÉE** : sept jours, chacun avec sa date et son état — `done`, `missed`, `before`, `upcoming`, `rest`, `free` — tous DÉDUITS, aucun stocké ; 400 tant que le programme n'a pas de date de début), `DELETE /programs/:id` (suppression logique rejouable) — plafond du plan gratuit à la création (`unlimited_programs`), voir le tableau détaillé ci-dessous | Post-Étape 7 ✅ |
...
| Communauté | **Livré** — `GET /community/feed`, `POST /community/encouragements` (201), `GET /community/friends`, `DELETE /community/friends/:userId` (204), `GET/POST /community/requests` (202 opaque, refus opposable 30 jours, seau dédié 10 demandes/min par adresse → 429), `POST /community/requests/:id/accept` et `…/decline` (204), `GET /community/challenges` (crée le jeu du mois à la première lecture, catalogue en code, idempotent sous concurrence), `POST` et `DELETE /community/challenges/:id/join`, `POST /community/quiz-answers` (204), `GET/PATCH /community/profile` (confidentialité) ; modération : `POST`/`DELETE /community/blocks/:userId` (204, blocage opaque dans les deux sens), `GET /community/blocks`, `DELETE /community/encouragements/:id` (204, auteur ou destinataire, rejouable), `POST /community/reports` (201 ; une personne, un encouragement reçu d'elle ou un défi entre amis qu'elle a créé, jamais les deux cibles à la fois) ; back-office : `GET /admin/community/reports?status=` et `PATCH /admin/community/reports/:id` (permission `community:moderate`, audité), consommées par la page Signalements de `apps/admin` (`/reports`) — voir `docs/product/community.md` | Post-Étape 7 ✅ |
| Repas | **Livré** — `POST /api/v1/nutrition/meals` (201, création idempotente par id appareil, date refusée dans le futur, quantité facultative **descriptive** — `quantity` + `quantityUnit` vont par paire et ne multiplient NI les kcal NI les macros), `GET /nutrition/meals` (journal entre deux instants, plage plafonnée à 366 jours et 10 000 lignes), `PATCH /nutrition/meals/:id` (correction sur place : un champ absent reste tel quel, un champ à `null` efface ce qu'on croyait savoir ; `name`, `kcal` et `eatenAt` refusent `null`), `DELETE /nutrition/meals/:id` (204) — alimente les kcal consommées de l'accueil ; `GET /nutrition/meals/:id` (un repas, composants compris, 404 indiscernable) ; `moment` (`BREAKFAST`, `LUNCH`, `DINNER`, `SNACK` ou `null`) ; repas **composé** : `components: [{ id, foodCode, quantityG }]` (`id` UUID de l'appareil, stable d'une correction à l'autre et unique dans le journal, 409 sinon ; 30 au plus, 1 à 5 000 g), le serveur prend l'instantané des aliments et **calcule** kcal, macros (`null` dès qu'un aliment l'ignore) et quantité en grammes, 400 si le corps porte aussi un total, 400 pour corriger à la main un repas composé (des totaux renvoyés à l'identique sont ignorés), une ligne désignée par son `id` garde son instantané même si l'aliment a quitté la base, `components: []` retire la composition en gardant les totaux ; toute correction se décide sous le verrou de la ligne du repas ; `components[].sourceVersion` et `meta.source` (mention CIQUAL) dès qu'un repas rendu porte des aliments — voir `docs/product/nutrition.md` | Post-Étape 7 ✅ |
| Photo d'un repas | **Livré** — `PUT /api/v1/nutrition/meals/:id/photo` (multipart, champ `file`, un seul fichier, `image/jpeg` déclaré ET prouvé par les octets sinon 415, 5 Mio au plus sinon 413, 20 dépôts par minute ; EXIF et position GPS, XMP, IPTC et commentaires retirés avant stockage ; rend le repas), `GET …/photo` (octets bruts `image/jpeg`, `Cache-Control: private, no-cache`, ETag fort, 304 sur `If-None-Match`), `DELETE …/photo` (204 idempotent) ; le repas expose `photo: { updatedAt } \| null`. Propriétaire seulement (404 indiscernable sinon). Stockage dans un bucket PRIVÉ distinct (`S3_PRIVATE_BUCKET`), jamais servi sans l'API ; effacée avec le repas et avec le compte, y compris quand la suppression croise le dépôt (404, objet neuf repris) ; orphelins repris par `dist/cli/meal-photos-sweep`, lancé chaque jour par la supervision — voir `docs/product/nutrition.md`, « Photo d'un repas » | Post-Étape 7 ✅ |
| Aliments | **Livré, base à charger** — `GET /api/v1/nutrition/foods?q=&limit=` (chaque mot doit figurer dans le nom normalisé, retirés exclus, « commence par » puis nom court ; valeurs pour 100 g ; `meta.source` = mention CIQUAL obligatoire et version), `GET /nutrition/foods/:code` (404 inconnu ou retiré) — table CIQUAL de l'Anses (Licence Ouverte Etalab 2.0) chargée par `dist/cli/ciqual-import`, **pas encore validée sur le vrai fichier** ni branchée dans le déploiement — voir `docs/product/nutrition.md`, « Base d'aliments » | Post-Étape 7, validation sur le fichier réel à faire |
