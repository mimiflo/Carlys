...
void main() {
  ...
  test(
      ...
      'sans le moindre appel réseau', () async {
  ...
  test('le dernier lancement est mémorisé, sans opération supplémentaire',
      () async {
    api.networkDown = true;
    final templateId = await savePushTemplate();
  ...
  test(
    ...
    expect((await workouts.watchActiveWorkout().first)!.session.id, sessionId);
  });

  test('au plus une séance en cours : le lancement respecte la règle existante',
      () async {
    api.networkDown = true;
    final templateId = await savePushTemplate();
