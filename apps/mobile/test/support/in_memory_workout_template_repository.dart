/// Dépôt de modèles de séance EN MÉMOIRE — doublure de test.
///
/// Ancien dépôt du mode démonstration, déménagé dans les tests quand ce mode
/// a été retiré : cinq suites s'en servaient déjà comme d'un fake — c'est la
/// seule implémentation complète du contrat sans Drift ni réseau, lancement
/// de séance compris (délégué au dépôt de séances, comme l'implémentation
/// réelle délègue au domaine séance).
library;
...
import 'package:carlys_mobile/features/workout_template/domain/services/template_normalizer.dart';
...
/// Jeu d'exemple (deux modèles réalistes), sous la forme d'entrées
/// d'écriture ordinaires : le dépôt les enregistre comme n'importe quelle
/// saisie. C'est le contenu de départ par défaut — plusieurs suites
/// s'appuient sur « Push force » et « Pull hypertrophie ».
List<SaveTemplateInput> sampleTemplateSeed() => [
  for (final (index, template) in _sampleTemplates.indexed)
    SaveTemplateInput(
      id: 'modele-echantillon-$index',
      name: template.$1,
...
///
/// [seed] permet de choisir le contenu de départ ; par défaut c'est le jeu
/// d'exemple ([sampleTemplateSeed]).
class InMemoryWorkoutTemplateRepository implements WorkoutTemplateRepository {
  InMemoryWorkoutTemplateRepository(
    ...
    for (final (index, input) in (seed ?? sampleTemplateSeed()).indexed) {
