...
**Étape 3 — Exercices : terminée.** Catalogue relationnel (Exercise, MuscleGroup, Equipment + liaisons rôles primaire/secondaire), **seed de 89 exercices** en français avec 12 groupes musculaires et 11 équipements, photos des pectoraux, des biceps et du dos comprises (`pnpm prisma:seed`, comptes de dev inclus), API `GET /exercises` avec recherche, filtres (groupe musculaire, équipement, difficulté, type) et **pagination par curseur**, fiche détaillée par id ou slug, référentiels `muscle-groups`/`equipment` — le tout servi à travers un **cache Redis tolérant aux pannes** (la base reste la source de vérité si Redis tombe). Côté Flutter : écran bibliothèque (recherche débouncée, puces de filtres, défilement infini), fiche d'exercice complète, nouveaux composants du design system (AppCard, AppSearchField, AppBadge). Tests unitaires, e2e et widgets.
