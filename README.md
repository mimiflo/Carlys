...
| `TRUST_PROXY_HOPS` | Nombre de proxys de confiance devant l'API (`0` en local, `2` en production : le proxy réseau puis le Nginx du serveur) — sans lui, la limitation de débit et l'audit ne voient que l'adresse du proxy (`docs/security/reverse-proxy.md`) | `0` |
...
| `./scripts/check_mobile.sh` | Vérifications Flutter — rejoue `mobile-ci` à l'identique, précédé du contrôle des tailles de fichiers (règle du dépôt, absente de la CI) |
| `./scripts/check_mobile_file_sizes.sh` | Seuls les seuils de taille (widgets, use cases, services) — appelé par le précédent |
...
**Étape 3 — Exercices : terminée.** Catalogue relationnel (Exercise, MuscleGroup, Equipment + liaisons rôles primaire/secondaire), **seed de 170 exercices** en français avec 12 groupes musculaires et 15 équipements, photos des pectoraux, des biceps, du dos, des triceps, des épaules et des abdominaux comprises (`pnpm prisma:seed`, comptes de dev inclus), API `GET /exercises` avec recherche, filtres (groupe musculaire, équipement, difficulté, type) et **pagination par curseur**, fiche détaillée par id ou slug, référentiels `muscle-groups`/`equipment` — le tout servi à travers un **cache Redis tolérant aux pannes** (la base reste la source de vérité si Redis tombe). Côté Flutter : écran bibliothèque (recherche débouncée, puces de filtres, défilement infini), fiche d'exercice complète, nouveaux composants du design system (AppCard, AppSearchField, AppBadge). Tests unitaires, e2e et widgets.
